# 만기매칭형(존속기한형) ETF 수급 트래커 - 개발 명세서 (Technical Specification)
...
본 시스템은 한국 주식시장에 상장된 **만기매칭형(Target-Maturity) 크레딧 채권 ETF**의 자금 유출입(Fund Flow)을 추적하고, 이를 잔존 듀레이션(Duration) 버킷별, 종목별로 분석하는 자동화 대시보드입니다. 

- **핵심 목표:** 가격 등락으로 인한 평가 손익(Capital Gain/Loss)의 착시를 제거하고, 실제 시장에서 ETF로 밀려 들어오고 나가는 **'순수한 자금 유입액(Real Net Flow)'**을 추적합니다.
...
- **역할:** 시장 데이터의 동적 탐색 및 수집
- **주요 로직 (Dynamic Discovery):**
  ...
  4. 네이버 차트 API(`fchart.stock.naver.com`)를 호출하여 과거 30일치 시계열(종가 등)을 수집하고, 라이브 API의 현재 상장좌수(Shares)를 결합하여 DB에 저장합니다.
...
- **주요 테이블 스키마:**
  ...
  - `daily_etf_raw`: 일별 원천 데이터 (상장좌수, NAV, AUM).
  - `duration_flow_daily`: 가공된 데이터 (잔존 듀레이션, 수급액, 3일/5일 이동평균).

### 2.3. 데이터 연산 프로세서 (`src/processor.py`)
- **역할:** 핵심 비즈니스 로직(Fund Flow 및 Duration) 연산
- **핵심 공식 (Core Formulas):**
  - **순유입액(Net Fund Flow):** `(오늘 상장좌수 - 어제 상장좌수) × 어제 NAV`
    *(어제 종가 기준으로 오늘 좌수가 늘어났다면, 그만큼의 현금이 지정참가회사(LP)를 통해 설정(Creation)되었다고 간주함)*
  - **잔존 듀레이션(Duration):** `(ETF 목표 만기일 - 연산 당일 날짜) ÷ 365.0`
  - **버킷 분류(Bucketing):** 잔존 듀레이션을 기준(0.5Y, 1.0Y, 1.5Y, 2.0Y 이상)으로 범주화하여 거시적 분석 레이어를 제공합니다.

### 2.4. 시각화 대시보드 (`app.py`)
- **역할:** Streamlit 기반 인터랙티브 UI 및 분석 뷰 제공
- **구조:** 종목(Ticker) 중심 뷰
  - 사이드바 필터를 통해 특정 기간 내 데이터를 동적으로 슬라이싱(Slicing) 및 랭킹 정렬(AUM순, 만기순)합니다.
  - 필터링된 기간 내의 일별 수급액을 `Sum()`하여 기간 누적 수급 랭킹을 즉각적으로 크로스섹션(Cross-sectional) 차트로 렌더링합니다.
...
- **목표:** 자금 유입(Fund Flow)과 크레딧 스프레드 축소/확대 간의 상관관계(Correlation) 분석.
- **개발 방안:**
  ...
  - `app.py`에서 두 데이터를 Merge하여 시계열을 겹쳐 그리면, "스프레드가 벌어질 때 만기매칭형으로 자금이 유입되는가?"에 대한 훌륭한 퀀트 전략 백테스팅 도구가 됩니다.
...
- 네이버 `sise_invest.naver` 웹 크롤링 또는 향후 pykrx 라이브러리가 복구될 경우, 일별 기관/외인 순매수 추이를 `daily_etf_raw`에 추가 컬럼으로 적재할 수 있습니다. 이를 통해 수급의 주체(Who) 분석이 가능해집니다.
